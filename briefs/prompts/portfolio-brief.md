...
1. **Lead with what moves money.** Market Snapshot and Top Stories should focus
   ...
   sector rotations, macro data releases.
2. **Connect stories to positions.** Every major story should be evaluated for
   how it affects held positions or watchlist items.
3. **Merge related items.** If Reuters and CNBC cover the same event, combine
   into one entry with multiple source links.
4. **Skip empty sections.** If a source category yielded no content, omit the
   section entirely instead of writing filler text.
