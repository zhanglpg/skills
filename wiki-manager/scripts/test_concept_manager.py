...
class TestFindConceptPage(unittest.TestCase):
    def setUp(self):
    ...
    def _write_concept(self, name, aliases=None):
        aliases_yaml = ""
        if aliases:
        ...
        (self.concept_dir / f"{name}.md").write_text(content)

    def test_find_by_stem(self):
        self._write_concept("Transformer")
        result = find_concept_page("Transformer", self.concept_dir)
        self.assertIsNotNone(result)
        self.assertEqual(result.stem, "Transformer")

    def test_find_by_alias(self):
        self._write_concept("RLHF", aliases=["Reinforcement Learning from Human Feedback"])
        result = find_concept_page("Reinforcement Learning from Human Feedback", self.concept_dir)
        self.assertIsNotNone(result)
    ...
    def test_not_found(self):
        result = find_concept_page("Nonexistent", self.concept_dir)
        self.assertIsNone(result)


class TestListConcepts(unittest.TestCase):
    def test_empty_dir(self):
        tmpdir = tempfile.mkdtemp()
        concept_dir = Path(tmpdir) / "concepts"
        concept_dir.mkdir()
        result = list_concepts(concept_dir)
        self.assertEqual(result, [])
        import shutil
        shutil.rmtree(tmpdir, ignore_errors=True)

    def test_lists_concepts(self):
        tmpdir = tempfile.mkdtemp()
        concept_dir = Path(tmpdir) / "concepts"
        concept_dir.mkdir()
        ...
        (concept_dir / "RLHF.md").write_text(
            '---\ntitle: "RLHF"\ntype: concept\n---\n\n# RLHF\n'
        )
        result = list_concepts(concept_dir)
...
class TestCreateConceptPage(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp()
        self.concept_dir = Path(self.tmpdir) / "concepts"
        self.concept_dir.mkdir()
    ...
    def test_creates_file(self):
        ...
        path = create_concept_page("Transformer", "digest content", self.concept_dir, mock_llm)
    ...
    def test_wraps_output_without_frontmatter(self):
        ...
        path = create_concept_page("Test", "digest content", self.concept_dir, mock_llm)
        content = path.read_text()
        self.assertTrue(content.startswith("---"))
        self.assertIn('title: "Test"', content)


class TestUpdateConceptPage(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp()
        self.concept_dir = Path(self.tmpdir) / "concepts"
        self.concept_dir.mkdir()
    ...
    def test_update_with_valid_output(self):
        ...
        concept_path = self.concept_dir / "Transformer.md"
        concept_path.write_text(original)
        ...
        update_concept_page(concept_path, "New Paper", "digest content", mock_llm)
...
class TestExtractConcepts(unittest.TestCase):
    def test_json_array_response(self):
        ...
        result = extract_concepts_from_digest("digest content", [], mock_llm)
        self.assertEqual(result, ["Transformer", "Attention Mechanism", "BERT"])

    def test_json_in_text(self):
        def mock_llm(prompt):
            return 'Here are the concepts:\n["Transformer", "RLHF"]\n'
    ...
    def test_max_concepts(self):
        ...
        result = extract_concepts_from_digest("digest content", [], mock_llm, max_concepts=3)
...
class TestCreateConceptPageSanitization(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp()
        self.concept_dir = Path(self.tmpdir) / "concepts"
        self.concept_dir.mkdir()
    ...
    def test_strips_fences_from_created_file(self):
        ...
        path = create_concept_page("Transformer", "digest", self.concept_dir, mock_llm)
        content = path.read_text()
        self.assertTrue(content.startswith("---"))
