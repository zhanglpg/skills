#!/usr/bin/env python3
"""Unit tests for generate_brief.py"""
...
class TestHashAndDeduplication(unittest.TestCase):
    ...
    def test_hash_article_deterministic(self):
        h1 = self.gen._hash_article("Title", "https://example.com")
        h2 = self.gen._hash_article("Title", "https://example.com")
        self.assertEqual(h1, h2)

    def test_hash_article_different_inputs(self):
        h1 = self.gen._hash_article("Title A", "https://a.com")
        h2 = self.gen._hash_article("Title B", "https://b.com")
    ...
    def test_hash_article_matches_md5(self):
        ...
        self.assertEqual(self.gen._hash_article(title, url), expected)

    def test_hash_article_empty_url(self):
        h = self.gen._hash_article("Title")
        expected = hashlib.md5("Title:".encode()).hexdigest()
        self.assertEqual(h, expected)

    def test_is_duplicate_first_seen(self):
        self.assertFalse(self.gen._is_duplicate("New Article", "https://new.com"))

    def test_is_duplicate_second_seen(self):
        self.gen._is_duplicate("Dupe", "https://dupe.com")
        self.assertTrue(self.gen._is_duplicate("Dupe", "https://dupe.com"))

    def test_is_duplicate_different_url_not_duplicate(self):
        self.gen._is_duplicate("Title", "https://a.com")
        self.assertFalse(self.gen._is_duplicate("Title", "https://b.com"))

    def test_seen_hashes_populated(self):
        self.gen._is_duplicate("Article", "https://example.com")
        self.assertEqual(len(self.gen.seen_hashes), 1)
...
class TestDateMethods(unittest.TestCase):
    ...
    def test_get_date_str_format(self, mock_dt):
        ...
        # With offset 8, 10:00 UTC -> 18:00 CST, same date
...
class TestGetSourcesByCategory(unittest.TestCase):
    ...
    def test_filter_newsletter(self):
        result = self.gen._get_sources_by_category('newsletter')
        self.assertEqual(len(result), 2)
        self.assertEqual(result[0]['name'], 'NL1')

    def test_filter_ai_lab(self):
        result = self.gen._get_sources_by_category('ai_lab')
        self.assertEqual(len(result), 1)

    def test_filter_nonexistent_category(self):
        result = self.gen._get_sources_by_category('nonexistent')
...
class TestXmlText(unittest.TestCase):
    def test_returns_text(self):
        ...
        self.assertEqual(gb.BriefGenerator._xml_text(root, 'child'), 'Hello')

    def test_returns_none_for_missing(self):
        import xml.etree.ElementTree as ET
        root = ET.fromstring('<root><child>Hello</child></root>')
        self.assertIsNone(gb.BriefGenerator._xml_text(root, 'missing'))

    def test_returns_none_for_empty_text(self):
        import xml.etree.ElementTree as ET
        root = ET.fromstring('<root><child></child></root>')
        self.assertIsNone(gb.BriefGenerator._xml_text(root, 'child'))

    def test_strips_whitespace(self):
        import xml.etree.ElementTree as ET
        root = ET.fromstring('<root><child>  spaced  </child></root>')
        self.assertEqual(gb.BriefGenerator._xml_text(root, 'child'), 'spaced')
...
class TestParseFeedXml(unittest.TestCase):
    ...
    def test_parse_rss20(self):
        articles = self.gen._parse_feed_xml(RSS_20_FEED, 'TestSource')
    ...
    def test_atom_alternate_link(self):
        articles = self.gen._parse_feed_xml(ATOM_FEED, 'Test')
        self.assertEqual(articles[0]['url'], 'https://example.com/atom/1')

    def test_atom_fallback_link(self):
        articles = self.gen._parse_feed_xml(ATOM_FEED, 'Test')
        # Second entry has no rel="alternate", falls back to first <link>
    ...
    def test_malformed_xml_returns_empty(self):
        articles = self.gen._parse_feed_xml(MALFORMED_XML, 'Bad')
        self.assertEqual(articles, [])

    def test_empty_feed_returns_empty(self):
        articles = self.gen._parse_feed_xml('<rss><channel></channel></rss>', 'Empty')
        self.assertEqual(articles, [])
    ...
    def test_caps_at_15_articles(self):
        ...
        articles = self.gen._parse_feed_xml(xml, 'Many')
    ...
    def test_summary_truncated_to_300(self):
        ...
        articles = self.gen._parse_feed_xml(xml, 'Trunc')
    ...
    def test_item_without_title_skipped(self):
        ...
        articles = self.gen._parse_feed_xml(xml, 'NoTitle')
...
class TestHttpGet(unittest.TestCase):
    ...
    @patch.object(gb, '_HAS_HTTPX', False)
    ...
    def test_urllib_fallback_success(self, mock_urlopen):
        ...
        result = self.gen._http_get('https://example.com')
    ...
    @patch.object(gb, '_HAS_HTTPX', True)
    def test_httpx_success(self):
        ...
        with patch.dict(sys.modules, {'httpx': mock_httpx}):
            # Inject the mock into the module namespace
            original = getattr(gb, 'httpx', None)
            gb.httpx = mock_httpx
            try:
                result = self.gen._http_get('https://example.com')
    ...
    def test_custom_timeout(self, mock_urlopen):
        ...
        self.gen._http_get('https://example.com', timeout=30)
...
class TestCheckAndFetchRss(unittest.TestCase):
    ...
    @patch.object(gb.BriefGenerator, '_http_get')
    def test_ok_and_failed_sources(self, mock_get):
        ...
        ok, failed = self.gen.check_and_fetch_rss()
    ...
    def test_articles_stored_in_fetched_content(self, mock_get):
        ...
        self.gen.config['rss_sources'] = [
            {'name': 'Feed', 'rss': 'https://feed.com/rss', 'category': 'newsletter'}
        ]
        self.gen.check_and_fetch_rss()
        self.assertGreater(len(self.gen.fetched_content['rss']), 0)
    ...
    def test_deduplication_across_feeds(self, mock_get):
        ...
        self.gen.check_and_fetch_rss()
        # Both feeds return the same articles, so dedup should keep only one set
        self.assertEqual(len(self.gen.fetched_content['rss']), 2)
    ...
    def test_source_coverage_updated(self, mock_get):
        ...
        self.gen.check_and_fetch_rss()
        self.assertIn('Tracked', self.gen.source_coverage)
        self.assertEqual(self.gen.source_coverage['Tracked'], 2)

    @patch.object(gb.BriefGenerator, '_http_get', return_value=None)
...
class TestFetchArxivPapers(unittest.TestCase):
    def setUp(self):
        self.gen = _make_generator({'arxiv_categories': ['cs.LG', 'cs.AI']})

    @patch.object(gb.BriefGenerator, '_http_get', return_value=ARXIV_RESPONSE)
    def test_parses_arxiv_entries(self, mock_get):
        papers = self.gen.fetch_arxiv_papers()
    ...
    @patch.object(gb.BriefGenerator, '_http_get', return_value=MALFORMED_XML)
...
class TestFetchHackerNews(unittest.TestCase):
    ...
    def test_fetches_and_filters_ai_stories(self, mock_get):
        ...
        stories = self.gen.fetch_hackernews_top(limit=10)
        # Should find at least the AI-related ones
    ...
    @patch.object(gb.BriefGenerator, '_http_get', return_value=None)
    def test_api_failure_returns_empty(self, mock_get):
        stories = self.gen.fetch_hackernews_top()
    ...
    def test_fallback_when_few_ai_stories(self, mock_get):
        ...
        stories = self.gen.fetch_hackernews_top(limit=10)
        # Since <3 AI stories, all stories included
...
class TestFetchGithubTrending(unittest.TestCase):
    ...
    def test_parses_github_response(self, mock_get):
        ...
        repos = self.gen.fetch_github_trending()
    ...
    @patch.object(gb.BriefGenerator, '_http_get', return_value='invalid json')
...
class TestFetchWebSource(unittest.TestCase):
    ...
    @patch.object(gb, '_HAS_TRAFILATURA', False)
    @patch.object(gb.BriefGenerator, '_http_get')
    def test_html_fallback_strips_tags(self, mock_get):
        ...
        result = self.gen.fetch_web_source('Test', 'https://test.com')
        self.assertIsNotNone(result)
        self.assertNotIn('<', result['content'])

    @patch.object(gb.BriefGenerator, '_http_get', return_value=None)
    def test_fetch_failure(self, mock_get):
        result = self.gen.fetch_web_source('Bad', 'https://bad.com')
        self.assertIsNone(result)

    @patch.object(gb, '_HAS_TRAFILATURA', False)
    @patch.object(gb.BriefGenerator, '_http_get', return_value='<html><body>short</body></html>')
    def test_short_content_returns_none(self, mock_get):
        result = self.gen.fetch_web_source('Short', 'https://short.com')
        self.assertIsNone(result)
    ...
    def test_content_capped_at_2000(self, mock_get):
        ...
        result = self.gen.fetch_web_source('Long', 'https://long.com')
        self.assertIsNotNone(result)
...
class TestRunGemini(unittest.TestCase):
    ...
    def test_success(self, mock_run):
        ...
        result = self.gen.run_gemini('test prompt')
    ...
    def test_failure_retries(self, mock_run):
        ...
        with patch('time.sleep'):
            result = self.gen.run_gemini('prompt', retry=1)
    ...
    def test_missing_binary(self, mock_run):
        with patch('time.sleep'):
            result = self.gen.run_gemini('prompt', retry=0)
        self.assertIn('Error', result)
...
class TestValidateBrief(unittest.TestCase):
    ...
    def test_valid_brief(self):
        ...
        self.assertTrue(self.gen.validate_brief(brief))

    def test_missing_heading(self):
        brief = "No headings here, just text https://example.com"
        # '#' is in 'https', so it actually needs both '#' and '##'
        # Actually '#' is not in this text. Let me check.
        # Wait, there's no '#' in this text so it should fail.
        self.assertFalse(self.gen.validate_brief(brief))
    ...
    def test_low_source_coverage_still_valid(self):
        # Has required elements but few source indicators
        brief = "# Title\n## Section\nhttps://example.com\nTwitter only"
        self.assertTrue(self.gen.validate_brief(brief))
...
class TestFormatFailedSourcesWarning(unittest.TestCase):
    ...
    def test_no_failed_sources(self):
        self.assertEqual(self.gen._format_failed_sources_warning([]), "")
    ...
    def test_with_failed_sources(self):
        ...
        result = self.gen._format_failed_sources_warning(failed)
...
class TestSourceCoverageReport(unittest.TestCase):
    def setUp(self):
        ...
        self.gen.fetched_content['rss'] = [{'title': 'a'}]
        self.gen.fetched_content['arxiv'] = [{'title': 'b'}, {'title': 'c'}]
        self.gen.fetched_content['hackernews'] = []
        self.gen.fetched_content['github_trending'] = [{'name': 'x'}]
        self.gen.fetched_content['web_pages'] = [{'source': 'WebSrc'}]

    def test_report_contains_data_counts(self):
        report = self.gen.generate_source_coverage_report()
...
class TestGeneratorInit(unittest.TestCase):
    def test_init_creates_empty_fetched_content(self):
        ...
        self.assertEqual(set(gen.fetched_content.keys()), expected_keys)
        for v in gen.fetched_content.values():
            self.assertEqual(v, [])

    def test_init_empty_seen_hashes(self):
        gen = _make_generator()
        self.assertEqual(len(gen.seen_hashes), 0)

    def test_init_empty_source_coverage(self):
        gen = _make_generator()
        self.assertEqual(gen.source_coverage, {})

    def test_init_empty_failed_sources(self):
        gen = _make_generator()
        self.assertEqual(gen.failed_sources, [])
...
class TestFetchWebSourcesParallel(unittest.TestCase):
    ...
    @patch.object(gb.BriefGenerator, 'fetch_web_source')
    def test_only_fetches_fetchable_categories(self, mock_fetch):
        ...
        self.gen.fetch_web_sources_parallel()
        # Should only fetch newsletter and ai_lab, not youtube or podcast
        self.assertEqual(mock_fetch.call_count, 2)

    @patch.object(gb.BriefGenerator, 'fetch_web_source', return_value=None)
    def test_handles_all_failures(self, mock_fetch):
        results = self.gen.fetch_web_sources_parallel()
...
class TestGenerateBriefContent(unittest.TestCase):
    ...
    @patch.object(gb.BriefGenerator, 'run_gemini', return_value='# Brief\n## Section\nhttps://example.com')
    def test_calls_gemini_with_prompt(self, mock_gemini):
        result = self.gen.generate_brief_content()
        mock_gemini.assert_called_once()
        prompt = mock_gemini.call_args[0][0]
        self.assertIn('summarizer', prompt)
        self.assertIn('@karpathy', prompt)

    @patch.object(gb.BriefGenerator, 'run_gemini', return_value='brief content')
    def test_includes_failed_sources_note(self, mock_gemini):
        failed = [{'name': 'BadSource', 'url': 'https://bad.com', 'error': 'timeout'}]
        self.gen.generate_brief_content(failed_sources=failed)
        prompt = mock_gemini.call_args[0][0]
